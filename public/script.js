        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            fields.forEach(function(field) {
                ...
                if (field.hasAttribute('required') && value === '') {
                    fieldValid = false;
                    message = field.getAttribute('data-error-required') || 'This field is required';
                }
                
                if (fieldValid && field.type === 'email' && value !== '') {
                    var emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                    if (!emailRegex.test(value)) {
                        fieldValid = false;
                        message = field.getAttribute('data-error-email') || 'Please enter a valid email address';
                    }
                }
                
                if (fieldValid && field.hasAttribute('minlength') && value !== '') {
                    var minLength = parseInt(field.getAttribute('minlength'));
                    if (value.length < minLength) {
                        fieldValid = false;
                        message = field.getAttribute('data-error-minlength') || 'Minimum ' + minLength + ' characters required';
                    }
                }
                
                if (fieldValid && field.hasAttribute('pattern') && value !== '') {
            ...
            if (isValid) {
                ...
                xhr.onload = function() {
                    ...
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        errorMsg.textContent = contactForm.getAttribute('data-error-message') || 'An error occurred. Please try again.';
                ...
                xhr.onerror = function() {
                    ...
                    var errorMsg = document.createElement('div');
                    errorMsg.className = 'form-error';
                    errorMsg.textContent = contactForm.getAttribute('data-error-message') || 'Network error. Please check your connection.';
                    contactForm.appendChild(errorMsg);
                    setTimeout(function() { errorMsg.remove(); }, 5000);
                };
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '62e65962-2d77-40d8-97cd-1a58cca35090',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '62e65962-2d77-40d8-97cd-1a58cca35090';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '62e65962-2d77-40d8-97cd-1a58cca35090';
